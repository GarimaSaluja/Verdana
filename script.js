...
async function getCareGuide() {
  ...
  try {
    ...
    const res = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json", "x-api-key": ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01", "anthropic-dangerous-direct-browser-access": "true" }, body: JSON.stringify({ model: "claude-haiku-4-5-20251001", max_tokens: 1000, messages }) });
    const data = await res.json();
...
async function sendChatMessage() {
  ...
  try {
    const res = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json", "x-api-key": ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01", "anthropic-dangerous-direct-browser-access": "true" }, body: JSON.stringify({ model: "claude-haiku-4-5-20251001", max_tokens: 1000, system: `You are Verdana AI, a friendly plant care expert. Help with plant identification, care, troubleshooting, watering, soil, and gardening tips. Be concise and warm. Use plant emojis. User's plants: ${myPlants.map(p => p.name).join(', ') || 'none yet'}.`, messages: chatHistory.slice(-10) }) });
    const data = await res.json();
